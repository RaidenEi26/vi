			...
			for (let name in global.plugins) {
				...
				if ((usedPrefix = (match[0] || '')[0])) {
					...
					if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
						this.reply(m.chat, `🐱 Your limit runs out, please buy through ${usedPrefix}buylimit, or contact the owner Anos Ai *+6281267905243*`, m)
						continue
					}
...
global.dfail = async (type, m, conn) => {
  let pp = await conn.profilePictureUrl(m.sender, 'image').catch(_ => 'https://cdn.jsdelivr.net/gh/SazumiVicky/Storage@main/anang.jpeg')
  let {
    data
