...
let handler = async (m, { conn, isPrems }) => {
    ...
    if (new Date - global.db.data.users[m.sender].lastclaim < 3600000) throw `You've already claimed, today's daily claims wait another *${msToTime(time - new Date())}*`
    global.db.data.users[m.sender].exp += isPrems ? prem : free
    global.db.data.users[m.sender].balance += isPrems ? balanceprem : balancefree
    m.reply(`• C O N G R A T U L A T I O N S\n\n◦ +${isPrems ? prem : free} Exp\n◦ +${isPrems ? balanceprem : balancefree} balance`)
    global.db.data.users[m.sender].lastclaim = new Date() * 1
    setTimeout(() => {
        m.reply(`🐱 Hourly is back`)
    }, timeout)
}
