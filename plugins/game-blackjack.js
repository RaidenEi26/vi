...
      // Mengubah nilai Ace menjadi 1 jika total melebihi 21
      if (hasAce && total > 21) {
        total -= 10;
      }
...
    // Mengambil kartu acak dari deck
    let pickCard = () => {
      let index = Math.floor(Math.random() * cards.length);
      return cards[index];
    };

    // Mulai game Blackjack
...
      // Cek Blackjack pada awal permainan
...
        // Jika tidak ada Blackjack pada awal permainan, lanjutkan permainan
...
      if (playerTotal > 21) {
        // Jika total kartu melebihi 21, player kalah
...
        delete conn.bj[m.sender];
      } else {
        // Lanjut ke giliran player berikutnya
        let message = `*• B L A C K J A C K*\n\n`;
        message += `╭── •\n`
        message += `│ ◦ *Kartu Kamu:* ${playerCards.join(', ')}\n`;
        message += `│ ◦ *Total Kamu:* ${playerTotal}\n`;
...
      // Giliran player selesai, giliran komputer
...
      if (computerTotal > 21) {
        // Jika total kartu komputer melebihi 21, player menang
        sendResultMessage(`*Kamu menang! Total kartu Komputer melebihi 21.*\n*+${bet} Balance*`, playerTotal, computerTotal);
        global.db.data.users[m.sender].balance += bet * 1;
      } else if (playerTotal > computerTotal) {
        // Jika total kartu player lebih besar, player menang
        sendResultMessage(`*Kamu menang!*\n*+${bet} Balance*`, playerTotal, computerTotal);
        global.db.data.users[m.sender].balance += bet * 1;
      } else if (playerTotal < computerTotal) {
        // kartu player lebih kecil, player kalah
        sendResultMessage(`*Kamu kalah!*\n*-${bet} Balance*`, playerTotal, computerTotal);
        global.db.data.users[m.sender].balance -= bet;
      } else {
        // Jika total kartu sama, hasil seri
